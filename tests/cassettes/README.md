...
**Every tape was re-recorded on 13 Sep 2026 on Vertex AI**: `orchestrator.json` from
`gemini-3.8-flash`, `correlator.json` and `proposer.json` from `gemini-3.1-pro-preview`, via
`scripts/record_cassettes.py` against the demo fixture alert. `injection/` is regenerated by
`scripts/record_injection_cassettes.py`. `writer_author.json` (W42 rung 3, `gemini-3.1-pro-preview`)
holds one writer per contract — a ConfigMap's `data` and `binaryData` — re-recorded 13 Sep ($0.0271)
after the prompt learned the second contract, by `scripts/record_writer_cassettes.py`, which rewrites
the tape from empty and refuses to report success unless every recorded writer passes the allowlist,
the probe and the template. `tests/e2e/test_containment.py` runs both against a real API server. The first recording (11 Sep) was
`gemini-3.5-flash-lite` on AI Studio; `agents/llm.py` says why the assignment changed.
Re-record with:

```bash
.venv/bin/python scripts/record_cassettes.py      # needs GEMINI_API_KEY in .env (a Vertex express key)
```
